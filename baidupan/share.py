...
def get_yun_data(surl: str, randsk: str):
    url = f'https://pan.baidu.com/s/1{surl}'
    res = requests.get(url, headers=api.get_randsk_headers(randsk), timeout=30).text
...
def get_file_list(shareid, uk, randsk, dir_str=None):
    ...
    return util.dict_to_object(json.loads(requests.get(url, headers=api.get_randsk_headers(randsk), timeout=30).text))
...
def handle_file_list(file_list, yun_data, randsk):
    ...
    for file in file_list.list:
        ...
        if dl_link:
            ...
            msg_file_str += '下载地址获取失败\n'
    return (f'找到以下目录(在原有的命令后加上{config.comm.split}目录进入)：\n' + '\n'.join(
        msg_dir_str) if msg_dir_str else '') + msg_file_str
